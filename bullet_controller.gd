...
func spawn_bullets_radial(bullet_count: int, shoot_radius: int, initial_direction:= Vector2.ZERO):
	...
	for i in range(-bullet_count/2, bullet_count/2):
		...
		var bullet : Bullet = _pool.get_object().initialize(self.global_position, dir, Globals.BulletShape.circle, "red") # if rng.randf() >= 0.95 else _pool.get_object(true).initialize(self.global_position, dir, Globals.BulletShape.circle, "red", true)
...
func spawn_bullets_faster(bullet_count: int, shoot_radius: int):
	...
	for i in range(-bullet_count/2, bullet_count/2+1):
		...
		var bullet : Bullet = _pool.get_object().initialize(self.global_position, dir) # if rng.randf() >= 0.95 else _pool.get_object(true).initialize(self.global_position, dir, true)
...
func spawn_bullets_sine(horizontal_speed: float, frequency: float, amplitude: float, speed := 400):
	...
	var bullet : Bullet = _pool.get_object().initialize(self.global_position, player_direction, Globals.BulletShape.capsule, "orange") # if rng.randf() >= 0.95 else _pool.get_object(true).initialize(self.global_position, player_direction, Globals.BulletShape.capsule, "orange",true)
	bullet.set_calculate_next_step(func(delta : float):
...
func spawn_constellation(shoot_radius: int, bullet_count: int):
	...
	var bullet : Bullet = _pool.get_object().initialize(self.global_position, dir, Globals.BulletShape.circle, "green") # if rng.randf() >= 0.95 else _pool.get_object(true).initialize(self.global_position, dir, Globals.BulletShape.circle, "green", true)
